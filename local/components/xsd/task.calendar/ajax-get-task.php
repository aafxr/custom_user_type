...
function getTasks($arFilter, &$arTasks) {
...
if($_REQUEST['showResponsible'] == 'Y') {
    ...
    $arFilter = array(
        ...
        'ONLY_ROOT_TASKS' => 'Y',
        'SAME_GROUP_PARENT' => 'Y',
        '::SUBFILTER-1' => $subFilter1
    );

    getTasks($arFilter,$arTasks);
}
...
/*
    if(($repeatedEndDate === false) || ($repeatedEndDate > $periodStart)) {
        ...
        while($repeatFlag) {
            ...
            $nextTime = CTasks::getNextTime(unserialize($arTemplate['~REPLICATE_PARAMS'], ['allowed_classes' => false]), $arTemplate['ID'], $nextTime); // localtime
