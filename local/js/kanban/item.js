...
BX.CRM.Kanban.Item.prototype = {
	...
	createLayout: function()
	{
		var gridData = this.getGridData();
		var className = '';	
		var uf = this.data.fields.find(el => el.code === 'UF_CRM_1611546509');
		this.data.fields = this.data.fields.filter(function(item) {
    			return item.code !== 'UF_CRM_1611546509'
		});
		
		if (uf && uf.value == '1') {
			className = ' ligr-green';
		} else 
		if (uf && uf.value == '2') {
			className = ' ligr-yellow';
		} else 
		if (uf && uf.value == '3') {
			className = ' ligr-red';
		}
		...
		this.container = BX.create("div", {
			props: {
				className: this.getGrid().getTypeInfoParam('kanbanItemClassName') + className,
			},
			events: {
				click: function(e)
				{
